...
import com.kobr4.tradebot.api.{BinanceApi, CurrencyPair, PoloOrder}
...
class BinanceApiTest extends FlatSpec with Matchers with ScalaFutures with BeforeAndAfterEach {
  ...
  it should "return open orders" in {
    ...
    wireMockServer.stubFor(get(urlEqualTo("/api/v3/openOrders"))
  ...
  it should "return trades history" in {
    ...
    wireMockServer.stubFor(get(urlEqualTo("/api/v3/myTrades"))
      .willReturn(aResponse()
        .withHeader("Content-Type", "text/plain")
        .withBody(
          """
