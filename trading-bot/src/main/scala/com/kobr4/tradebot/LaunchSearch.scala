...
object LaunchSearch extends StrictLogging {
  ...
  val date = ZonedDateTime.parse("2018-11-01T01:00:00.000Z")
  val endDate = ZonedDateTime.parse("2019-01-09T01:00:00.000Z")
  ...
  def main(args: Array[String]): Unit = {
    ...
    eventualPData.map { priceMap =>
      ...
      */
      val strategies = RuleGenerator.getAll(2).combinations(2).toList.flatMap { buyList =>
        RuleGenerator.getAll(2).combinations(2).toList.map { sellList =>
          GeneratedStrategy(buyList, sellList)
        }
      }
      var count = 0
