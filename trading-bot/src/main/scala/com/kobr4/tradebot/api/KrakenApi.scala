...
object KrakenCurrencyPairHelper {
...
case class KrakenTrade(ordertxid: String, pair: String, `type`: String, price: BigDecimal, vol: BigDecimal, time: Long) {
  ...
  def toOrder: Order = this.`type` match {
    case "buy" =>
      val currencyPair = KrakenCurrencyPairHelper.fromString(pair)
      Buy(
        currencyPair,
...
class KrakenApi(
  ...
  def returnTicker()(implicit ec: ExecutionContext): Future[List[Quote]] =
    tradableAsset.map(_.filter(aList =>
      (aList.contains("ETH") || aList.contains("XMR") || aList.contains("XBT") || aList.contains("XRP") || aList.contains("USDT")) && aList.contains("ZUSD")).mkString(",")).flatMap { assetListParam =>
      {
      ...
      KrakenApi.ReturnOpenOrders.build(reqNonce), apiKey, apiSecret).map { message =>
        Json.parse(message).as[JsObject].value("result").as[JsObject].value("open").as[JsObject].value.toList.map {
          case (txid, jsOrder) =>
            ...
            val pair = KrakenCurrencyPairHelper.fromString(jsOrder.as[JsObject].value("descr").as[JsObject].value("pair").as[String])
            PoloOrder(pair, txid, rate, vol)
        }
      }
  }
